...
with open(sys.argv[1],'r') as f: #lexicons
...
with open(sys.argv[2],'r') as f: #morphotactic rules
    morph = f.read().strip()
    morph = morph.split('\n')
...
for rule in morph :
    ...
    if pos in lex_map.keys():
        ...
        for i in range(len(lexemes)):
            ...
            new_rule = '('+state1+ ' (' + new_pos_state + ' "'+lexemes[i][0]+'" "'+lexemes[i][0]+'"))'
            op.append(new_rule)
            prev_state = new_pos_state
            for j in range(1,len(lexemes[i])):
                new_lex_state = new_pos_state + '_'+str(j)
                new_rule = '('+prev_state+ ' (' + new_lex_state + ' "'+lexemes[i][j]+'" "'+lexemes[i][j]+'"))'
                op.append(new_rule)
                prev_state = new_lex_state
            new_rule = '('+prev_state+ ' (' + state2 + ' *e* "'+'#'+pos+'*"))'
            op.append(new_rule)
    else:
        op.append(rule)
